...
3. What session variables are set after a successful login?
    Session variable 'user_id' is set to $user['id'] and session variable 'full_name' is set to $user['full_name'].
...
4. When you click “Add to Cart,” what exactly gets stored in $_SESSION['cart']? Which action adds items to the cart, and what type of data is being stored?
    If the session variable 'cart' is  set, session variable cart is set to $record_id.' This is a foreign key pointing to the records table.
...
5. On the cart page, you use $records_in_cart. Where does that variable come from, and why do we need records_by_ids() instead of just using the raw IDs in the session?
    We want to displaye the record details to the user, not the ids.
...
6. Explain what happens when you click “Complete Purchase.” Which action in index.php runs, what loop is executed, which function writes each record to the database, and which table is updated?
    Complete purchase runs the switch case of 'checkout.' 
    
    First it calls the funcion require_login(); that returns void. 
    then it sets the '$cart_ids' array varriable to either the cart of the session, or an empty array if session cart isn't set. 
    
    Then there is an if statement that checks if '$cart_ids' is set.
    
    If true, for each of the cart ids it calls the 'purchase_create()' function which runs a sequel statement that adds the selected purchases to the purchases table.
    Else, the cart is set to an empty array. 

    View is changed to 'checkout_success.'
